...
import net.suncaper.springboot.domain.BuyerCart;
import net.suncaper.springboot.domain.BuyerItem;
...
@Controller
public class BuerCartController {
    @Autowired
    ...
    public <T> String buyerCart(Long skuId, Integer amount, HttpServletRequest request,
        ...
        String username = UserService.getAttributterForUsername(RequestUtils.getCSessionId(request, response));
        if (null != username) {
            //登录了
            //4, 将购物车追加到Redis中
            UserService.insertBuyerCartToRedis(buyerCart, username);
    ...
    public String toCart(Model model, HttpServletRequest request,
        ...
        if (null != username) {
            ...
            //3, 取出Redis中的购物车
            buyerCart = UserService.selectBuyerCartFromRedis(username);
        }
        ...
        if(items.size() > 0){
            ...
            for (BuyerItem buyerItem : items) {
                buyerItem.setSku(UserService.selectSkuById(buyerItem.getSku().getId()));
            }
        }
    ...
    public String trueBuy(String[] skuIds, Model model, HttpServletRequest request, HttpServletResponse response){
        ...
        String username = UserService.getAttributterForUsername(RequestUtils.getCSessionId(request, response));
        //取出所有购物车
        BuyerCart buyerCart = UserService.selectBuyerCartFromRedisBySkuIds(skuIds, username);
        List<BuyerItem> items = buyerCart.getItems();
        if (items.size() > 0) {
            //购物车中有商品
